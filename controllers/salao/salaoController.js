...
const createSalao = async (req, res) => {
  try {
...
const deleteSalao = async (req, res) => {
  try {
    const salao = await Salao.findByIdAndDelete({ _id: req.params.id });
    if (!salao) {
      return res.status(404).json({ msg: "Salão não encontrado" });
    }
...
const updateSalao = async (req, res) => {
  try {
    const salao = await Salao.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
const getSalaoById = async (req, res) => {
  try {
    const salao = await Salao.findById({ _id: req.params.id });
    if (!salao) {
      return res.json({ msg: "Salão não encontrado" });
    }
